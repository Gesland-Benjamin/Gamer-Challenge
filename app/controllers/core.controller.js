export class CoreController {
  render404(req, res) {
    ...
    // je voudrais afficher le messaege d'erreur dans la console
    res.status(404).render("error", { errorCode: 404, message: "Page non trouvée" });
  };
  render400(req, res) {
  ...
  render403(req, res) {
    res.status(403).render("error", { errorCode: 403, message: "Nom d'utilisateur ou mot de passe incorrect" });
  };
  render401(req, res) {
    res.status(401).render("error", { errorCode: 401, message: "Non autorisé" });
  };
  render500(req, res) {
